...
class TacotronDecoder(nn.Module):
    ...
    def forward(self, src_mask, encoder_out, decoder_inputs, hid = None,**kwargs):
     ...
     #   decoder_inputs = self.linear_trans(decoder_inputs)




        decoder_output, hid = self.lstm(decoder_inputs, hid)


        lt = decoder_inputs.size(1)

        attn_mask = src_mask.unsqueeze(1).expand(-1, lt, -1)

        decoder_output = self.preprocess_layer(decoder_output)

        decoder_output = decoder_output.transpose(0, 1).contiguous()
        encoder_out = encoder_out.transpose(0, 1).contiguous()


        output, alignments = self.multihead_tgt(decoder_output, encoder_out, encoder_out, attn_mask)

        output = self.postprocess_layer(output)

        decoder_output = self.linear_projection(output)
        gate_prediction = self.gate_layer(output)

        mel_outputs, gate_outputs, alignments = self.parse_decoder_outputs(
            decoder_output[:-1], gate_prediction[:-1], alignments)

        return mel_outputs, gate_outputs.squeeze(2) , alignments
